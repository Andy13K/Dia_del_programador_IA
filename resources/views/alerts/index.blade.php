...
<div class="space-y-6">
    ...
    <section class="kin-panel p-4 sm:p-6">
        <form method="GET" action="{{ route('alerts.index') }}" class="flex flex-col sm:flex-row gap-3 mb-6">
            <div class="flex-1"><label for="alert-farm" class="block text-xs mb-2">Granja solar</label><select id="alert-farm" name="solar_farm_id" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-transparent px-3 text-sm"><option value="">Todas las granjas</option>@foreach($farms as $farm)<option value="{{ $farm->id }}" @selected(($filters['solar_farm_id'] ?? '') == $farm->id)>{{ $farm->name }}</option>@endforeach</select></div>
            <div class="sm:w-48"><label for="alert-status" class="block text-xs mb-2">Estado</label><select id="alert-status" name="status" class="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-transparent px-3 text-sm"><option value="">Todos los estados</option><option value="active" @selected(($filters['status'] ?? '') === 'active')>Activa</option><option value="resolved" @selected(($filters['status'] ?? '') === 'resolved')>Resuelta</option></select></div>
