...
| `Presentacion-Kin-Solar-Guatemala.pptx` | `node build-presentacion-pptx.js` (requiere `npm install pptxgenjs` en una carpeta aparte). Genera un PowerPoint editable de 15 diapositivas, alternando tema oscuro/claro con los tokens de color reales del sistema, con capturas reales del sistema (`docs/evidencias/pptx/`), la tabla Estándar → Artefacto (ERS, ERD, OWASP, ISO 25010), la estructura completa de `docs/`, el flujo de ramas/PRs/commits, y una única diapositiva de "Módulos del Sistema" como transición hacia la demostración en vivo — pensado como respaldo editable de `public/presentacion.html`. |
