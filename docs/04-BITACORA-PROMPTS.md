...
**Pendiente para el equipo:** `docs/export/ERS-Kin-Solar-Guatemala.docx`/`.pdf` quedan desactualizados frente al Markdown corregido — el script de regeneración (`build-ers-docx.js`) tiene una ruta absoluta hardcodeada a la copia de trabajo principal (no a este worktree), así que no se ejecutó desde aquí para no escribir en un checkout ajeno a esta sesión. Falta correr `node build-ers-docx.js` (y reimprimir `ers-print.html` a PDF) desde la copia principal después de mergear este PR. La copia del `.docx` que el humano tiene en Descargas ya fue parchada a mano con las mismas 4 correcciones como solución inmediata, pero no sustituye la regeneración desde la fuente.
